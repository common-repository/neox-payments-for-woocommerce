...
abstract class WC_NeoX_Abstract extends WC_Payment_Gateway
{
    ...
    public function get_pay_url($order)
    {
        ...
        foreach ( $order->get_items() as $item_id => $item ) {
            $orderData['payItems'][] = array(
                'orderId' => $item->get_product_id(),
                'desc' => $item->get_name(),
        ...
        $order_notes = wc_get_order_notes(array('order_id' => $order->get_id(), 'orderby' => 'date_created_gmt', 'order' => 'DESC'));
        if ($order_notes) {
            $customNoteContent = $order_notes[0]->content;
        }

        $srcPlatformData = array(
            "neoxPluginVersion" => '1.0.7',
            "platformName" => 'WORDPRESS',
            "platformVersion" => get_bloginfo('version'),
            "wcVersion" => WC_VERSION,
            "orderKey" => $order->get_order_key(),
            "orderNoteCustomer" => $order->get_customer_note(),
            "orderNoteHistory" => $order_notes,
            "orderCancleUrl" => $order->get_cancel_order_url_raw()
        );

        $neo_ExtData = array(
            "srcPlatformData" => $srcPlatformData,
            "orderData" => $orderData,
        );
        ...
        $args = array(
            ...
            'neo_OrderInfo' => !empty($customNoteContent) ? $customNoteContent : (!empty($order->get_customer_note()) ? $order->get_customer_note() : $order->get_id()),
            'neo_PaymentMethod' => $methods,
            'neo_ReturnURL' => $order->get_checkout_order_received_url(),
            'neo_CancelURL' => $order->get_cancel_order_url_raw(),
            'neo_Title' => 'Payment',
            'neo_Version' => 1,
            'neo_ExtData' => json_encode($neo_ExtData)
        );
